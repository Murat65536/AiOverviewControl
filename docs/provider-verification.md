...
| xAI (Grok Build / SuperGrok) | Grok CLI billing — `GET https://cli-chat-proxy.grok.com/v1/billing?format=credits` (same surface the official CLI uses after `grok login`) | Weekly or monthly `creditUsagePercent` plus period end from `config.currentPeriod`. An omitted percent on a parseable period is zero usage. Credentials: `~/.grok/auth.json` (or `$GROK_HOME`). Plan label from `/v1/settings` → `subscription_tier_display`. |
...
CI runs the fixture-backed suites on every push (see `.github/workflows/ci.yml`).
