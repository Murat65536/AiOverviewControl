...
| **Adapter** | `fetch_xai_native` — prefers Grok CLI billing, renews refreshable OIDC sessions through `grok models`, then falls back to Management API prepaid balance and `GET /v1/api-key` auth-only. It retries billing once only for `401` or `403`. |
